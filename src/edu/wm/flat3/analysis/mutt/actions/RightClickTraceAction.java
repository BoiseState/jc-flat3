...
{	
	...
	public void run(IAction action)
	{
		...
		if (selection instanceof CompilationUnit) {
				...
				String args[] = {};//{"harder.txt"}; // TODO, run dialog: let user set these args
		
				// Store the last trace so that user can reinvoke it via the trace button
				// TODO: Store these across sessions? 
				FLATTT.lastTraceDir = directory.toOSString();
				FLATTT.lastTraceClassName = className;
				FLATTT.lastTraceArgs = args;
				FLATTT.lastTraceProject = unit.getJavaProject();
